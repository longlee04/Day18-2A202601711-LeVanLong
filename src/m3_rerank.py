...
from config import RERANK_TOP_K, MODEL_DEVICE
...
class CrossEncoderReranker:
    ...
    def _load_model(self):
        if self._model is None:
            ...
            self._model = CrossEncoder(self.model_name, device=MODEL_DEVICE)
        return self._model

    def rerank(self, query: str, documents: list[dict], top_k: int = RERANK_TOP_K) -> list[RerankResult]:
        """Rerank documents: top-20 → top-k."""
        # Trả sớm khi không có candidate → không load model, không crash
        if not documents:
            return []
