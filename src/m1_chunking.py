...
from config import (DATA_DIR, HIERARCHICAL_PARENT_SIZE, HIERARCHICAL_CHILD_SIZE,
                    SEMANTIC_THRESHOLD, MODEL_DEVICE)
...
def _get_semantic_encoder():
    """Cache SentenceTransformer để không load lại model mỗi lần gọi."""
    global _SEMANTIC_ENCODER
    if _SEMANTIC_ENCODER is None:
        from sentence_transformers import SentenceTransformer

        _SEMANTIC_ENCODER = SentenceTransformer("all-MiniLM-L6-v2", device=MODEL_DEVICE)
...
def _split_to_size(text: str, max_size: int) -> list[str]:
    ...
    current = ""
    for unit in units:
        # Câu dài hơn max_size → cắt cứng theo ranh giới từ
        while len(unit) > max_size:
            cut = unit.rfind(" ", 0, max_size)
            if cut <= 0:
                cut = max_size
            if current:
...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        if cosine_sim(embeddings[i - 1], embeddings[i]) < threshold:
            groups.append([sentences[i]])       # đổi chủ đề → cắt chunk mới
        else:
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    # 1. Gộp paragraph thành parent (mỗi parent ≤ parent_size chars)
    ...
    for pi, ptext in enumerate(parent_texts):
        ...
        parents.append(Chunk(
            ...
            metadata={**metadata, "chunk_type": "parent", "parent_id": pid,
                      "strategy": "hierarchical", "chunk_index": pi},
        ))

        # 2. Mỗi parent → children ≤ child_size chars, cắt theo ranh giới câu
        for ci, ctext in enumerate(_split_to_size(ptext, child_size)):
            children.append(Chunk(
                text=ctext,
                metadata={**metadata, "chunk_type": "child", "parent_id": pid,
                          "strategy": "hierarchical", "chunk_index": ci},
                parent_id=pid,
            ))
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    # Giữ nguyên dòng heading trong output nhờ capture group của re.split
    ...
    for part in parts:
        if re.match(r"^#{1,3}\s+.+$", part or "", flags=re.MULTILINE):
            _flush()                 # đóng section trước đó
            current_header = part.strip()
        else:
            buffer += part or ""     # body giữ nguyên table/list/code block
    _flush()
